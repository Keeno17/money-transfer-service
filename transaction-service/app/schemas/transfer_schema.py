...
class CreateTransferRequest(BaseModel):
    from_account: uuid.UUID = Field(
        ..., description="The UUID of the account to transfer from"
    )
    to_account: uuid.UUID = Field(
        ..., description="The UUID of the account to transfer to"
    )
    amount: Decimal = Field(
        ..., gt=0, description="The amount to transfer (must be greater than 0)"
    )
